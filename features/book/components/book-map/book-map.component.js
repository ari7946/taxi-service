...
const Map = ({ locationsCleared, locationsFound, routeChanged }) => {
	useEffect(() => {
		...
		script.onload = () => {
			...
			const unitSelector = tomtom.unitSelector.getHtmlElement(
			...
			routeInputsInstance.on(
				routeInputsInstance.Events.LocationsFound,
				(eventObject) => {
			...
			routeInputsInstance.on(
				routeInputsInstance.Events.LocationsCleared,
				(eventObject) => {
